...
static unsigned int ATD_gc2155_status;
static unsigned int who_am_i = 2155;
static struct i2c_client *g_client;
static u16 g_by_file = 0x0;
static int exposure_return0;
static unsigned int read_register_page;
static unsigned int read_register;

static ssize_t gc2155_show_status(struct device *dev,
...
static int gc2155_set_streaming(struct v4l2_subdev *sd)
{
	...
	gc2155_write_reg(client, GC2155_8BIT,  0x01, 0x87);
#else
...
static int gc2155_init_common_by_file(struct v4l2_subdev *sd)
{
	...
	fp = filp_open("/data/data/gc2155.txt", O_RDONLY, 0);
	...
	if (!IS_ERR_OR_NULL(fp)) {
		...
		*/
		...
		pr_err("%s: /data/data/gc2155.txt not found error\n", __func__);
		ret = -EINVAL;
	} else {
		pr_err("%s: /data/data/gc2155.txt open error\n", __func__);
		ret = -EINVAL;
	}
				...
				const struct gc2155_res_struct *res)
{
	...
	/* gc2155 spec: buf->line_length_pck = hb + sh_delay + buf->output_width/2 + 4;
				  ...
				  struct v4l2_mbus_framefmt *fmt)
{
	...
	case GC2155_RES_QCIF:
		pr_info("gc2155_set_mbus_fmt: QCIF\n");
	...
	case GC2155_RES_VGA:
		...
		break;
	case GC2155_RES_1600x900:
		pr_info("gc2155_set_mbus_fmt: 1600x900\n");
					...
					struct atomisp_exposure *exposure)
{
	...
	/* exposure time is 13bit only */
       if (coarse_integration > 8191) {
		coarse_integration = 8191;
		pr_info("%s, exposure time larger than max, set to 8192\n", __func__);
       }
	/*
...
static ssize_t dbg_set_gc2155_reg_read(
	struct file *file,
	char __user *buf,
	size_t count,
	loff_t *ppos)
{
	...
	/* set to wanted page */
	err = gc2155_read_reg(g_client, GC2155_8BIT,  REG_RST_AND_PG_SELECT, &tmp);
	pr_info("%s, page = %d\n", __func__, tmp);
	err = gc2155_write_reg(g_client, GC2155_8BIT, REG_RST_AND_PG_SELECT, read_register_page);
	err = gc2155_read_reg(g_client, GC2155_8BIT,  REG_RST_AND_PG_SELECT, &tmp);
	pr_info("%s, page = %d\n", __func__, tmp);

	/* read register */
	read_register = read_register & 0xFF;
	err = gc2155_read_reg(g_client, GC2155_8BIT,
					read_register, &value);
	pr_info("%s, read [0x%x] = 0x%x\n", __func__, read_register, value);

	/* set back to page0 */
	err = gc2155_write_reg(g_client, GC2155_8BIT, REG_RST_AND_PG_SELECT, 0x0);
	err = gc2155_read_reg(g_client, GC2155_8BIT,  REG_RST_AND_PG_SELECT, &tmp);
	pr_info("%s, page = %d\n", __func__, tmp);
...
static int gc2155_dbgfs_init(void)
{
	...
	debugfs_create_u32("vga_status", 0644, debugfs_dir, &ATD_gc2155_status);

	(void) debugfs_create_file("i2c_write_read", S_IRUGO | S_IWUSR,
		debugfs_dir, NULL, &dbg_set_gc2155_reg_fops);
	...
	debugfs_create_u32("read_register_page", 0644, debugfs_dir, &read_register_page);
	debugfs_create_u32("read_register", 0644, debugfs_dir, &read_register);

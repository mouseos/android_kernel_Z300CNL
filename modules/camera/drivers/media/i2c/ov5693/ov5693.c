...
static unsigned int WhoAmI = 5693;
				    ...
				    int w, int h)
{
	...
	pr_info("%s: idx= %d; select %s\n", __func__, idx,
		dev->ov5693_res[idx].desc);
	return idx;
}
				...
				int w, int h)
{
	...
	for (i = 0; i < dev->curr_res_num; i++) {
		if (w != dev->ov5693_res[i].width) {
			pr_info("ov5693: %s: skip i=%d, because of %s width\n",
				__func__, i, dev->ov5693_res[i].desc);
			continue;
		}
		if (h != dev->ov5693_res[i].height) {
			pr_info("ov5693: %s: skip i=%d, because of %s height\n",
				__func__, i, dev->ov5693_res[i].desc);
			continue;
		}
		return i;
			     ...
			     struct v4l2_mbus_framefmt *fmt)
{
	...
	pr_info("%s: get_resolution_index(%d, %d)\n", __func__,
	...
	pr_info("%s: set resolution: %s\n", __func__,
		dev->ov5693_res[dev->fmt_idx].desc);
...
static void ov5693_read_otp(struct i2c_client *client)
{
	int err = 0;
	int i = 0, j = 0;

	/*
	 * Make Sure sensor is not work at standby mode ( 0x0100 = 0) before
	 * writing or reading data.
	 */
	err = ov5693_write_reg(client, OV5693_8BIT, OV5693_SW_STREAM, 0x01);
	msleep(15);

	for (j = 3; j > 0; j--) {
		/* set bank */
		err |= ov5693_write_reg(client, OV5693_8BIT, 0x3D84, 0xC0 + j);
		err |= ov5693_write_reg(client, OV5693_8BIT, 0x3D81, 0x01);
		...
		for (i = 0; i < 10; i++) {
			...
			err |= ov5693_read_reg(client,
				OV5693_8BIT, 0x3D00 + i, &otp_val);
			otp[i] = otp_val;
		}

		if (otp[9] != 0x0) {
			pr_info("%s(): bank[%d] is ok\n", __func__, j);
			pr_info("otp[0~4]= {0x%X 0x%X 0x%X 0x%X 0x%X}\n",
					otp[0], otp[1], otp[2], otp[3], otp[4]);
			pr_info("otp[5~9]= {0x%X 0x%X 0x%X 0x%X 0x%X}\n",
					otp[5], otp[6], otp[7], otp[8], otp[9]);
			break;
		} else if (j == 1)
			pr_info("%s(): bank[1] is invalid. No AF Data?\n",
				__func__);
		else
			pr_info("%s(): bank[%d] is invalid. Find the next...\n",
				__func__, j);
	}
...
static int ov5693_detect(struct i2c_client *client)
{
	...
	/* Read OTP data and store it in otp[]. */
	ov5693_read_otp(client);
			   ...
			   int irq, void *platform_data)
{
	...
	if (dev->platform_data->platform_init) {
		...
		if (ret) {
			v4l2_err(client, "ov5693 platform init err\n");
			return ret;
		}
	}
...
/* ++++ dbgfs ++++ */
static int dbg_ov5693_camera_status_open(
	...
	loff_t *ppos)
{
	...
	/* Read Chip Id */
	err = ov5693_read_reg(g_client, OV5693_16BIT,
					OV5693_SC_CMMN_CHIP_ID, &id);
...
static int dbg_dump_ov5693_otp_open(struct inode *inode, struct file *file)
{
	file->private_data = inode->i_private;
	return 0;
}
...
static int dbg_set_ov5693_reg_open(struct inode *inode, struct file *file)
{
	file->private_data = inode->i_private;
	return 0;
}
	...
	loff_t *ppos)
{
	...
	*b_ptr++ = reg_val;
	buf_count++;
	val = reg_val;
	ov5693_write_reg(g_client, OV5693_8BIT, i2c_reg, val);
...
static int ov5693_dbgfs_init(void)
{

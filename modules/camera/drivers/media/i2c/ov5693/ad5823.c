...
static int ad5823_i2c_write(struct i2c_client *client, u8 reg, u8 val)
{
	...
	if (i2c_transfer(client->adapter, &msg, 1) != 1) {
		dev_err(&client->dev, "%s write [%x]:%x failed\n",
			__func__, reg, val);
		return -EIO;
	}
	return 0;
...
static int ad5823_i2c_read(struct i2c_client *client, u8 reg, u8 *val)
{
	...
	*val = 0;
	if (i2c_transfer(client->adapter, msg, 2) != 2) {
		dev_err(&client->dev, "%s read [%x] failed\n",
			__func__, reg);
		return -EIO;
	}
	*val = buf[1];
...
#ifdef ASUS_VCM_INIT_SETTINGS
		ret = ad5823_i2c_write(client, 0x01, 0x00);
		ret |= ad5823_i2c_write(client, 0x02, 0x10);
		ret |= ad5823_i2c_write(client, 0x03, 0x59);
		ret |= ad5823_i2c_write(client, 0x06, 0x00);
		ret |= ad5823_i2c_write(client, 0x07, 0x00);
		ret |= ad5823_i2c_write(client, 0x04, 0x05);
		ret |= ad5823_i2c_write(client, 0x05, 0x2C);

		if (ret) {
			pr_info("%s()--: vcm init error\n", __func__);
			return ret;
		}
#else

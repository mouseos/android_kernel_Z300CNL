			 ...
			 struct atomisp_dvs2_bq_resolutions *bq_res)
{
	...
	if ((!asd->continuous_mode->val) && (pipe_cfg->bayer_ds_out_res.width == 0)) {
		...
		if (asd->continuous_mode->val
			...
			isp_sink_crop.height =
				max_t(unsigned int, f->fmt.pix.height,
				      isp_sink_crop.height);
		} else if (stream_config->input_config.input_res.width * 1000 / f->fmt.pix.width < ATOMISP_CROP_RATIO) {
			isp_sink_crop.width = f->fmt.pix.width;
			isp_sink_crop.height = f->fmt.pix.height;
		} else {
			if (isp_sink_crop.width * f->fmt.pix.height >
				f->fmt.pix.width * isp_sink_crop.height) {
				isp_sink_crop.width = f->fmt.pix.width * isp_sink_crop.height / f->fmt.pix.height;
			} else if (isp_sink_crop.width * f->fmt.pix.height <
				f->fmt.pix.width * isp_sink_crop.height) {
				isp_sink_crop.height = f->fmt.pix.height * isp_sink_crop.width / f->fmt.pix.width;
			}
			dev_dbg(isp->dev,
				"should not crop too much, so cropped resolution is not set to output size\n");
		}

...
static struct mdm_info *drv_info;
...
static ssize_t req_cdump_show(struct device *dev, struct device_attribute *attr, char *buf)
{
...
void ap_req_cdump_init(void *drv, void *data)
{
	...
	if (mdm_cpu->gpio_ap_req_cdump > 0) {
		...
		if (ret) {
			...
			ret = device_create_file(drv_info->dev, &dev_attr_req_cdump);
			if (ret)
...
void ap_req_cdump_exit(void)
{
	...
	device_remove_file(drv_info->dev, &dev_attr_req_cdump);

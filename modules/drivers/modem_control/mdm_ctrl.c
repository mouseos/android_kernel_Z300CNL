...
static int mdm_ctrl_cold_boot(struct mdm_info *mdm)
{
	...
	pr_info(DRVNAME ": Cold boot requested");
...
static int mdm_ctrl_module_probe(struct platform_device *pdev)
{
	...
	mdm_drv->all_pdata->cpu.sim_det_init = sim_detect_init;
	mdm_drv->all_pdata->cpu.sim_det_exit = sim_detect_exit;

	mdm_drv->all_pdata->cpu.ap_req_cdump_init = ap_req_cdump_init;
	mdm_drv->all_pdata->cpu.ap_req_cdump_exit = ap_req_cdump_exit;

	mdm_drv->all_pdata->cpu.sim_det_init(mdm_drv, mdm_drv->all_pdata->cpu_data);
	mdm_drv->all_pdata->cpu.ap_req_cdump_init(mdm_drv, mdm_drv->all_pdata->cpu_data);
...
static int mdm_ctrl_module_remove(struct platform_device *pdev)
{
	...
	mdm_drv->all_pdata->cpu.sim_det_exit();
	mdm_drv->all_pdata->cpu.ap_req_cdump_exit();

 ...
 *
 */
int mdm_ctrl_configure_gpio(int gpio,
					  int direction,
...
EXPORT_SYMBOL_GPL(mdm_ctrl_configure_gpio);
...
int cpu_init_gpio(void *data)
{
	...
	/* Configure the MODEM_RSVD_GPIO2 gpio */
	ret = mdm_ctrl_configure_gpio(cpu_data->gpio_modem_rsvd_gpio2, 1, 0, "ModemControl_MODEM_RSVD_GPIO2");
	if (ret)
		goto free_ctx1;

	pr_info(DRVNAME
		": GPIO (rst_bbn: %d, pwr_on: %d, rst_out: %d, "
		"fcdp_rb: %d, sim1_ap_cd: %d, modem_rsvd_gpio2: %d)\n",
		cpu_data->gpio_rst_bbn, cpu_data->gpio_pwr_on,
		cpu_data->gpio_rst_out, cpu_data->gpio_cdump,
		cpu_data->gpio_sim1_ap_cd, cpu_data->gpio_modem_rsvd_gpio2);

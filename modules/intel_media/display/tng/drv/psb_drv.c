...
bool mrst_get_vbt_data(struct drm_psb_private *dev_priv)
{
	...
	switch (project) {
		...
		case 8:
			/* Z300CL */
			dev_priv->panel_id = CPT_CLAT101WR61XG_VID;
			break;
		case 10:
			/* Z300CNL */
			dev_priv->panel_id = BOE_TV101WXMNU1_VID;
			break;
		default:
			DRM_ERROR("No match project id.\n");
			dev_priv->panel_id = INNOLUX_N070ICEG02_VID;
			break;
	}
...
static int hist_control_read(struct file *file, char __user *buf,
				size_t nbytes,loff_t *ppos)
{
	return 0;
}

extern int psb_hist_enable(struct drm_device *dev, void *data);
extern int hist_enable;
static int hist_control_write(struct file *file, const char *buffer,
				      size_t count, loff_t *ppos)
{
	char buf[2];
	int  hist_control;
	...
	if (count != sizeof(buf)) {
		...
		if (buf[count-1] != '\n')
			return -EINVAL;
		hist_control = buf[0] - '0';
		PSB_DEBUG_ENTRY(" hist control: %d\n", hist_control);

		switch (hist_control) {
		case 0x0:
		case 0x1:
			hist_enable = hist_control;
			psb_hist_enable(dev, &hist_control);
			printk("Set hist_enable as %d\n", hist_control);
			break;
		default:
...
static const struct file_operations psb_hist_proc_fops = {
       .owner = THIS_MODULE,
       .read = hist_control_read,
       .write = hist_control_write,
};
...
static int psb_proc_init(struct drm_minor *minor)
{
	struct proc_dir_entry *csc_setting;
        struct proc_dir_entry *hist_setting;
        
        hist_setting = proc_create_data("dpst_hist_ctl", 0644, minor->proc_root, &psb_hist_proc_fops, minor);
#ifdef CONFIG_SUPPORT_HDMI
...
extern int __init lp8557i_i2c_init(void);
...
extern void __exit lp8557i_i2c_exit(void);

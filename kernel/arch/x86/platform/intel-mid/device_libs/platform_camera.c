...
int project_id_for_cam = PROJECT_ID_INVALID;
int g_rear_cam_id =  CAM_REAR_INVALID;
int g_front_cam_id =  CAM_FRONT_INVALID;
...
const struct intel_v4l2_subdev_id v4l2_ids[] = {
	...
	{"ov2680", RAW_CAMERA, ATOMISP_CAMERA_PORT_SECONDARY},
					...
					struct devs_id *dev)
{
	...
	g_front_cam_id = asustek_get_camera_front();
	g_rear_cam_id = asustek_get_camera_rear();
	pr_info("%s(%d): cam id: {rear, front}={%d, %d}\n",
		__func__, __LINE__, g_rear_cam_id, g_front_cam_id);
...
#define MSIC_VPROG3_MRFLD_ON_2_90       0xC1    /* 2.90V and Auto mode */
	...
	if (intel_mid_identify_cpu() == INTEL_MID_CPU_CHIP_ANNIEDALE) {
		if (voltage == CAMERA_1_05_VOLT) {
			...
			return intel_scu_ipc_iowrite8(MSIC_VPROG3_MRFLD_CTRL,
			       ...
			       MSIC_VPROG_MRFLD_OFF);
		} else if (voltage == CAMERA_2_9_VOLT) {
                        return intel_scu_ipc_iowrite8(MSIC_VPROG3_MRFLD_CTRL,
                               flag ? MSIC_VPROG3_MRFLD_ON_2_90 :
                               MSIC_VPROG_MRFLD_OFF);
		} else {

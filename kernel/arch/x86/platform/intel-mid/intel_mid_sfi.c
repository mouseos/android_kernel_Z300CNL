...
/*Add for get firmware version by settings app*/
struct sfi_table_oemb *temp_g_oemb;
struct switch_dev iafw_sdev;
static ssize_t asus_iafw_version_show(struct switch_dev *sdev, char *buf);
...
/*add for get firmware version*/
static ssize_t asus_iafw_version_show(struct switch_dev *sdev, char *buf)
{
	return sprintf(buf, "%x.%x\n", temp_g_oemb->ifwi_major_version,
		temp_g_oemb->ifwi_minor_version);
}
...
static int __init sfi_parse_oemb(struct sfi_table_header *table)
{
	...
	/*add for get firmware version by app settings*/
	temp_g_oemb = (struct sfi_table_oemb *) table;
	iafw_sdev.name = "ia_fw";
	iafw_sdev.print_name = asus_iafw_version_show;
	if (switch_dev_register(&iafw_sdev) < 0)
		printk(KERN_INFO "switch_dev_register failed!\n");
...
#ifdef CONFIG_SFI_PCB
    sfi_parsing_done(sfi_table_parse(SFI_SIG_OEM1, NULL, NULL, sfi_parse_oem1) ? false : true);
#endif

						     ...
						     unsigned short device)
{
	...
	/* For MOFD ignore D0i0 on LSS 5 & 36*/
	if (platform_is(INTEL_ATOM_MOORFLD)){
                if((sub_sys_index == 0x0)&&(sub_sys_pos == 0x5))
                        return true;
		if ((sub_sys_index == 0x2) && (sub_sys_pos == 0x4))
                        return true;
	}
...
retry:
	...
	if (unlikely(chk_val != new_value)) {
		WARN_ON(1);
		printk(KERN_WARNING "LSS#%u: chk_val(0x%x) != new_value(0x%x)\n",
			(sub_sys_index * 16 + sub_sys_pos), chk_val, new_value);
	}
...
static void put_driverless_devices_in_d0i3(void)
{
	struct pci_dev *pdev = NULL;

	for_each_pci_dev(pdev) {
		/* In case, device doesn't have driver, put it in D0i3 */
...
{
	project_id  project_id = asustek_get_project_id();

	put_driverless_devices_in_d0i3();

	if (project_id == 8 || project_id == 10) {
		/* set LSS#36 to D3 state*/

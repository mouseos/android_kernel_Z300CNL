...
#define REG_MODE_MASK		0x06
#define REG_PLD_MASK		0x08

#define MODEL_SHIFT		1
#define MODEL_AUTO		0x00
#define MODEL_NORMAL		0x01
#define MODEL_ECO		0x02
...
static int intel_pmic_reg_setmode(struct regulator_dev *rdev, unsigned int mode)
{
	struct intel_pmic_info *pmic_info = rdev_get_drvdata(rdev);
	int ret;
	u8 reg;
	...
	case REGULATOR_MODE_NORMAL:
		reg |= MODEL_NORMAL << MODEL_SHIFT;
	case REGULATOR_MODE_IDLE:
	case REGULATOR_MODE_STANDBY:
	default:
		reg |= MODEL_AUTO << MODEL_SHIFT;
	}

	/* alwasy disable internal pull-down */
	reg |= PLD_DIS << PLD_SHIFT;

	dev_dbg(&rdev->dev, "%s: mode: %d, reg = %x\n",
		__func__, mode, reg);
	return intel_scu_ipc_iowrite8(pmic_info->pmic_reg, reg);
}

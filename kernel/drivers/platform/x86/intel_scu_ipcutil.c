...
struct osnib_reset_src {
	u8 id;
	const char *resetsrc_name;
};

static struct osnib_reset_src osnib_reset_src0[] = {
	{ 0x01, "Shutdown due to THERMTRIP#" },
	{ 0x02, "Shutdown due to PMIC Critical Temperature event" },
	{ 0x04, "Shutdown due to violation of the VCRIT threshold" },
	{ 0x08, "Shutdown due to user removing the battery" },
	{ 0x10, "Shutdown due to watchdog timer expiring" },
};

static struct osnib_reset_src osnib_reset_src1[] = {
	{ 0x01, "Shutdown due to COLDOFF" },
	{ 0x02, "Shutdown due to COLDRESET" },
	{ 0x04, "Shutdown due to WARMRESET" },
	{ 0x08, "Shutdown due to power button" },
	{ 0x10, "Shutdown due to WDT_RESET" },
	{ 0x20, "Shutdown due to PLATFORMRST" },
};
...
char* asus_uts_boot_up_reason = NULL;
char* asus_uts_reset_reason_src0 = NULL;
char* asus_uts_reset_reason_src1 = NULL;

int get_asus_uts_boot_up_reason(char* reason, int len)
{
	if(asus_uts_boot_up_reason)
		strncpy(reason, asus_uts_boot_up_reason, len);
	else
		return 1;
...
static void set_asus_uts_power_off_reason(u8 reset_ev1, u8 reset_ev2)
{
	int i = 0;

	if(reset_ev1) {
		for(i = 0; i < ARRAY_SIZE(osnib_reset_src0); i++) {
			if(reset_ev1 == osnib_reset_src0[i].id)
			{
				asus_uts_reset_reason_src0 = (char*) osnib_reset_src0[i].resetsrc_name;
			}
		}
	}
	else
		asus_uts_reset_reason_src0 = NULL;

	if(reset_ev2) {
		for(i = 0; i < ARRAY_SIZE(osnib_reset_src1); i++) {
			if(reset_ev2 == osnib_reset_src1[i].id)
			{
				asus_uts_reset_reason_src1 = (char*) osnib_reset_src1[i].resetsrc_name;
			}
		}
	}
	else
		asus_uts_reset_reason_src1 = NULL;

	return;
}

int get_asus_uts_reset_src0(char* resetsrc, int len)
{
	if(asus_uts_reset_reason_src0)
		strncpy(resetsrc, asus_uts_reset_reason_src0, len);
	else
		return 1;
...
int get_asus_uts_reset_src1(char* resetsrc, int len)
{
	if(asus_uts_reset_reason_src1)
		strncpy(resetsrc, asus_uts_reset_reason_src1, len);
	else
		return 1;
...
#define MSIC_VPROG3_MRFLD_ON    0xC1    /* 2.90V and Auto mode */
...
static int oshob_init(void)
{
	...
	for (i = 0; i < ARRAY_SIZE(osnib_wake_srcs); i++) {
		if (osnib_wake_srcs[i].id == wakesrc) {
			...
			asus_uts_boot_up_reason = (char*) (osnib_wake_srcs[i].wakesrc_name);
	...
	for (i = 0; i < ARRAY_SIZE(chip_reset_events); i++) {
		if (chip_reset_events[i].id == oshob_info->platform_type) {
			...
			set_asus_uts_power_off_reason(reset_ev1, reset_ev2);

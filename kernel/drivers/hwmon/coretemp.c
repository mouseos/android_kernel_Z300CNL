...
struct platform_data {
	...
	struct device_attribute continue_dump_attr;
	struct delayed_work dump_cpu_temp;
	bool continue_dump;
};
...
static ssize_t show_continue_dump(struct device *dev,
			struct device_attribute *devattr, char *buf)
{
...
static ssize_t store_continue_dump(struct device *dev,
			...
			const char *buf, size_t count)
{
	...
	pdata->continue_dump = !!val;

	cancel_delayed_work_sync(&pdata->dump_cpu_temp);
	if (pdata->continue_dump)
		schedule_delayed_work(&pdata->dump_cpu_temp, 2*HZ);
...
static int create_dump_attr(struct platform_data *pdata,
				      struct device *dev)
{
	sysfs_attr_init(&pdata->continue_dump_attr.attr);
	pdata->continue_dump_attr.attr.name = "continue_dump";
	pdata->continue_dump_attr.attr.mode =   S_IRUGO | S_IWUSR;
	pdata->continue_dump_attr.show = show_continue_dump;
	pdata->continue_dump_attr.store = store_continue_dump;
	return device_create_file(dev, &pdata->continue_dump_attr);
}
...
void dump_cpu_temp_work(struct work_struct *work)
{
	struct platform_data *pdata =  container_of(work,
				struct platform_data, dump_cpu_temp.work);
	int cpu_temp[4] = {1000};
	struct temp_data *tdata;
	u32 eax, edx;
	u32 i;
       ...
       for (i = 0; i < 4 ; i++) {
		tdata =  pdata->core_data[i + BASE_SYSFS_ATTR_NO];
		//printk("show_temp  show_temp coretemp pdata->core_data[%d]=%p\n", i + BASE_SYSFS_ATTR_NO, pdata->core_data[i + BASE_SYSFS_ATTR_NO]);
		mutex_lock(&tdata->update_lock);
	...
	printk("coretemp cpu0=%d cpu1=%d  cpu2=%d cpu3=%d\n", cpu_temp[0], cpu_temp[1], cpu_temp[2], cpu_temp[3]);

	if (pdata->continue_dump)
		schedule_delayed_work(&pdata->dump_cpu_temp, 5*HZ);
	return ;
}
...
static int coretemp_probe(struct platform_device *pdev)
{
	...
	err = create_dump_attr(pdata, &pdev->dev);
	if (err)
		printk("err: coretem_probe: create_dump_attr fail!\n");

	pdata->continue_dump = false;
	INIT_DELAYED_WORK(&pdata->dump_cpu_temp,  dump_cpu_temp_work) ;
	schedule_delayed_work(&pdata->dump_cpu_temp, 5*HZ);
	return 0;
...
static int coretemp_suspend(struct platform_device *pdev, pm_message_t state)
{
	struct platform_data *pdata = platform_get_drvdata(pdev);
...
static int coretemp_resume(struct platform_device *pdev)
{
	struct platform_data *pdata = platform_get_drvdata(pdev);

	if (pdata->continue_dump)
		schedule_delayed_work(&pdata->dump_cpu_temp, HZ);
	return 0;
}
...
static struct platform_driver coretemp_driver = {
	...
	.suspend=coretemp_suspend,
	.resume =coretemp_resume,
};

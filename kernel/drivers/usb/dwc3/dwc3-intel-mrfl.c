...
static const char *get_usb_phy_events_string(enum usb_phy_events event)
{
	switch (event) {
		case USB_EVENT_NONE:
		...
		case USB_EVENT_DRIVE_VBUS:
			return "USB_EVENT_DRIVE_VBUS";
		default:
			return "Undefined";
	}
}
		...
		unsigned long event, void *data)
{
	...
	printk(KERN_INFO "%s usb phy event %s +++\n", __func__, get_usb_phy_events_string(event));

	if (!otg)
		return NOTIFY_BAD;

	valid_chrg_type = POWER_SUPPLY_CHARGER_TYPE_USB_SDP |
		POWER_SUPPLY_CHARGER_TYPE_USB_CDP | POWER_SUPPLY_CHARGER_TYPE_USB_DCP |
		POWER_SUPPLY_CHARGER_TYPE_ACA_DOCK | POWER_SUPPLY_CHARGER_TYPE_SE1;

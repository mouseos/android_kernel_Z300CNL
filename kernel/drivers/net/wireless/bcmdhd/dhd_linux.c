...
const char *dhd_get_custom_nvram_path(void)
{
	const char *nvram_path_TW = "/etc/nvram_TW.txt";
	const char *nvram_path_JP = "/etc/nvram_JP.txt";
	...
	if (strcmp(ccode, "TW") == 0)
		return nvram_path_TW;
	else if (strcmp(ccode, "JP") == 0)
		return nvram_path_JP;
...
#ifdef GET_CUSTOM_NVRAM_PATH
	nv = dhd_get_custom_nvram_path();
#endif /* GET_CUSTOM_NVRAM_PATH */

...
static s32 gup_init_panel(struct goodix_ts_data *ts)
{
	...
	ret =
	    gtp_i2c_read_dbl_check(ts->client, GTP_REG_CONFIG_DATA, &opr_buf[0],
				   1);
...
static s8 gup_update_config(struct i2c_client *client)
{
	...
	/* check cfg len, add by Ue, this bug happen when PID/VID not correct and memory out of speace  */
	if (ts->gtp_cfg_len < GTP_CONFIG_MIN_LENGTH) {
		GTP_DEBUG
		    ("CFG len small then min len, auto equ 186 + 2 bytes.");
		ts->gtp_cfg_len = GTP_CONFIG_MAX_LENGTH + GTP_ADDR_LENGTH;
	}
	...
	file_config =
	    (u8 *) kzalloc(chip_cfg_len + GTP_ADDR_LENGTH, GFP_KERNEL);

...
struct platform_soc_data {
	...
	int irq;
	struct device_attribute soc_temp_dump_attr;
	struct delayed_work dump_soc_temp;
	bool continue_dump;
};
...
struct workqueue_struct *dump_soc_temp_workqueue;
...
static ssize_t show_soc_continue_dump(struct device *dev,
			struct device_attribute *devattr, char *buf)
{
	struct platform_soc_data *pdata = dev_get_drvdata(dev);

	return sprintf(buf, "%d\n", pdata->continue_dump);
}

static ssize_t store_soc_continue_dump(struct device *dev,
			...
			const char *buf, size_t count)
{
	...
	cancel_delayed_work_sync(&pdata->dump_soc_temp);
	if (pdata->continue_dump)
		schedule_delayed_work(&pdata->dump_soc_temp, 2*HZ);

	return count;
}

static int create_soc_dump_attr(struct platform_soc_data *pdata,
				      struct device *dev)
{
	sysfs_attr_init(&pdata->soc_temp_dump_attr.attr);
	pdata->soc_temp_dump_attr.attr.name = "soc_temp_dump";
	pdata->soc_temp_dump_attr.attr.mode =   S_IRUGO | S_IWUSR;
	pdata->soc_temp_dump_attr.show = show_soc_continue_dump;
	pdata->soc_temp_dump_attr.store = store_soc_continue_dump;
	return device_create_file(dev, &pdata->soc_temp_dump_attr);
}

void dump_soc_temp_work(struct work_struct *work)
{
	struct platform_soc_data *pdata =  container_of(work,
				struct platform_soc_data, dump_soc_temp.work);
	long cur_temp;

	mutex_lock(&thrm_update_lock);
	get_max_temp(pdata, &cur_temp);
	mutex_unlock(&thrm_update_lock);

	//printk(" %s soc  cur_temp = %l\n", ret ? " ": "error", ret ? cur_temp : ret);
	printk(KERN_INFO "soc cur_temp = %ld \n", cur_temp);

	if (pdata->continue_dump)
		queue_delayed_work(dump_soc_temp_workqueue, &pdata->dump_soc_temp, 5*HZ);
	return ;
}
...
static int soc_thermal_probe(struct platform_device *pdev)
{
	...
	create_soc_dump_attr(pdata, &pdev->dev);
	dump_soc_temp_workqueue = create_singlethread_workqueue("dump_soc_temp_workqueue");
	INIT_DELAYED_WORK(&pdata->dump_soc_temp,  dump_soc_temp_work);
	queue_delayed_work(dump_soc_temp_workqueue, &pdata->dump_soc_temp, 5*HZ);
	return 0;
#ifdef CONFIG_SENSORS_SOCDTS_INTERRUPT
exit_cdev:
...
static int soc_thermal_suspend(struct platform_device *pdev, pm_message_t message)
{
	struct platform_soc_data *pdata = platform_get_drvdata(pdev);
...
static int soc_thermal_resume(struct platform_device *pdev)
{
	...
	if (pdata->continue_dump)
		schedule_delayed_work(&pdata->dump_soc_temp, HZ);
	return 0;
}

...
struct thermal_data {
	...
	bool is_initialized;
	void *thrm_addr;
	struct device_attribute moor_temp_dump_attr;
	struct delayed_work dump_moor_temp;
	bool continue_dump;
};
static struct thermal_data *tdata;

struct workqueue_struct *dump_moor_temp_workqueue;
...
static ssize_t show_moor_continue_dump(struct device *dev,
...
static ssize_t store_moor_continue_dump(struct device *dev, struct device_attribute *devattr, const char *buf, size_t count)
{
	struct thermal_data *tdata = dev_get_drvdata(dev);
	long val;
	...
	tdata->continue_dump = !!val;

	cancel_delayed_work_sync(&tdata->dump_moor_temp);
	if (tdata->continue_dump)
		schedule_delayed_work(&tdata->dump_moor_temp, 2*HZ);

	return count;
}

static int create_soc_dump_attr(struct thermal_data *tdata, struct device *dev)
{
	sysfs_attr_init(&tdata->moor_temp_dump_attr.attr);
	tdata->moor_temp_dump_attr.attr.name = "moor_temp_dump";
	tdata->moor_temp_dump_attr.attr.mode =   S_IRUGO | S_IWUSR;
	tdata->moor_temp_dump_attr.show = show_moor_continue_dump;
	tdata->moor_temp_dump_attr.store = store_moor_continue_dump;
	return device_create_file(dev, &tdata->moor_temp_dump_attr);
}

void dump_moor_temp_work(struct work_struct *work)
{
	struct thermal_data *tdata =  container_of(work, struct thermal_data, dump_moor_temp.work);
	long temp[PMIC_THERMAL_SENSORS];
	int i, ret;

	for (i = 0; i < PMIC_THERMAL_SENSORS ; i++)
		ret = show_temp(tdata->tzd[i], &temp[i]);

	printk(KERN_INFO "SYSTHERM0 = %ld  SYSTHERM1 = %ld SYSTHERM2 = %ld PMIC_DIE = %ld\n", temp[0], temp[1], temp[2], temp[3]);

	if (tdata->continue_dump)
		queue_delayed_work(dump_moor_temp_workqueue, &tdata->dump_moor_temp, 5*HZ);
...
static int moor_thermal_probe(struct platform_device *pdev)
{
	...
	tdata->continue_dump = false;
	create_soc_dump_attr(tdata, &pdev->dev);
	dump_moor_temp_workqueue = create_singlethread_workqueue("dump_moor_temp_workqueue");
	INIT_DELAYED_WORK(&tdata->dump_moor_temp,  dump_moor_temp_work);
	queue_delayed_work(dump_moor_temp_workqueue, &tdata->dump_moor_temp, 5*HZ);
...
static int moor_thermal_resume(struct device *dev)
{
	...
	if (tdata->continue_dump)
		schedule_delayed_work(&tdata->dump_moor_temp, HZ);

...
static irqreturn_t sdhci_pci_sd_cd(int irq, void *dev_id)
{
	...
	pr_info("%s: carddetect_irq gpio_%d:%d\n", mmc_hostname(host->mmc),
		slot->cd_gpio, gpio_get_value(slot->cd_gpio));
...
static int intel_moor_emmc_probe_slot(struct sdhci_pci_slot *slot)
{
	...
	if (current_project != 0x8 && current_project != 0xa)
		slot->host->mmc->caps2 |= MMC_CAP2_CACHE_CTRL;
...
static int force_shutdown_notify(struct notifier_block *self, unsigned long action, void *dev)
{
	struct mmc_host *host = container_of(self, struct mmc_host, shutdown_notifier);
	project_id current_project = asustek_get_project_id();

	if (!strcmp("mmc0", mmc_hostname(host)) &&
		(current_project == 0x8 || current_project == 0xa)) {
		pr_info("mmc shutdown notify+\n");
		mmc_force_poweroff_notify(host);
		pr_info("mmc shutdown notify-\n");
	}
        return NOTIFY_OK;
}
	...
	int slotno)
{
	...
	host->mmc->shutdown_notifier.notifier_call = force_shutdown_notify;
	host->mmc->shutdown_notifier.priority = 100;
	register_reboot_notifier(&host->mmc->shutdown_notifier);

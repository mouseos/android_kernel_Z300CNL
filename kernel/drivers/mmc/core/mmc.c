...
static void _mmc_asusevent_log(struct  mmc_host *host)
{
	...
	printk("%s:[EMMC_STATUS] vendor=%s, emmc_version=%s, emmc_size=%dG,fw_version=0x%x, lifeA=0x0%x,lifeB=0x%x\n",
	...
	ASUSEvtlog("%s:[EMMC_STATUS] vendor=%s, emmc_version=%s, emmc_size=%dG,fw_version=0x%x, lifeA=0x0%x,lifeB=0x%x\n",
		mmc_hostname(host), vendor, emmc_version, host->card->mmc_total_size, host->card->cid.prv,
		host->card->ext_csd.raw_device_life_time_A, host->card->ext_csd.raw_device_life_time_B);

}

static void mmc_asusevent_log(struct mmc_host *host)
{
	int err = 0;
	u8 *asus_ext_csd = NULL;

	err = mmc_get_ext_csd(host->card, &asus_ext_csd);
	if(err || asus_ext_csd == NULL) {
		pr_info("%s [EMMC_STATUS] mmc_asusevent_log error=%d\n",mmc_hostname(host), err);
	} else {
...
static int mmc_read_ext_csd(struct mmc_card *card, u8 *ext_csd)
{
	...
	card->ext_csd.raw_device_life_time_PRE_EOL = ext_csd[267];
...
MMC_DEV_ATTR(life_time_pre_EOL, "0x%02x\n", card->ext_csd.raw_device_life_time_PRE_EOL);
MMC_DEV_ATTR(emmc_health, "%02x-%02x-%02x\n", card->ext_csd.raw_device_life_time_PRE_EOL, card->ext_csd.raw_device_life_time_A,card->ext_csd.raw_device_life_time_B);
...
static struct attribute *mmc_std_attrs[] = {
	...
	&dev_attr_life_time_pre_EOL.attr,
	NULL,
};
	...
	struct mmc_card *oldcard)
{
	...
	project_id current_project = asustek_get_project_id();
	BUG_ON(!host);
	...
	if ((card->ext_csd.rev >= 6) && (current_project == 0x8 || current_project == 0xa)) {
		pr_notice("Enableing PON feature\n");
                err = mmc_switch(card, EXT_CSD_CMD_SET_NORMAL,
                                 EXT_CSD_POWER_OFF_NOTIFICATION,
                                 EXT_CSD_POWER_ON,
                                 card->ext_csd.generic_cmd6_time);

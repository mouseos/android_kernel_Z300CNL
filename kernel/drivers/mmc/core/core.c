			...
			unsigned short vdd_bit)
{
	...
	if (vdd_bit) {
		...
		if (result == 0 && !mmc->regulator_enabled) {
			/* Prior to enabling vmmc, checking vqmmc first. */
			if (mmc->supply.vqmmc) {
		...
		/* After disabled vmmc, powering vqmmc off as well */
		if (mmc->supply.vqmmc) {
...
void mmc_force_poweroff_notify(struct mmc_host *host)
{
	...
	pr_info("claiming host for force poweroff notify\n");
	mmc_claim_host(host);
...
void mmc_rescan(struct work_struct *work)
{
	...
	/* remove redundent sd init if card is not present */
	if (!strcmp("mmc1", mmc_hostname(host)) && sdhci->ops->get_cd
		&& sdhci->ops->get_cd(sdhci) == 0) {
		mmc_claim_host(host);
		mmc_power_off(host);
		mmc_release_host(host);
		goto out;

...
char messages[256];
char boot_reason[100] = {0};
char boot_src0[100] = {0};
char boot_src1[100] = {0};
...
static void do_write_event_worker(struct work_struct *work)
{
	...
	if (IS_ERR((const void *)(long int)g_hfileEvtlog)) {
		...
		if(!*boot_reason)
			get_asus_uts_boot_up_reason(boot_reason, sizeof(boot_reason));

		if(!*boot_src0)
			get_asus_uts_reset_src0(boot_src0, sizeof(boot_src0));

		if(!*boot_src1)
			get_asus_uts_reset_src1(boot_src1, sizeof(boot_src1));

		snprintf(buffer, sizeof(buffer),
			"\n\n---------------System Boot----%s---------\n"
			"###### Bootup Reason: %s ######\n"
			"--------------- Power off : reset_src0 : %s ---------------\n"
			"--------------- Power off : reset_src1 : %s ---------------\n",
			ASUS_SW_VER,
			boot_reason,
			boot_src0,
			boot_src1);

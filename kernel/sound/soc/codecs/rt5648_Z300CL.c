...
static int rt5648_hal_mode;
static int rt5648_mic_mode;
...
static struct rt5648_init_reg init_list[] = {
	...
	{ RT5648_HP_VOL		, 0x8b8b },/* OUTMIX -> HPVOL */
...
#endif
	{ RT5648_SPK_VOL	, 0x8b8a },
	{ RT5648_DAC1_DIG_VOL	, 0xadad },
	/* record */
	...
	{ RT5648_SPO_CLSD_RATIO	, 0x0005 },
	{ RT5648_ADJ_HPF1, 0xa220 },
	{ RT5648_ADJ_HPF2, 0x0707 },
...
const unsigned int drc_pr_index[] = {
	0x94,
	0x95, 0x96, 0x97,
	0x98, 0x99, 0x9a,
	0x9b, 0x9c, 0x9d
...
const unsigned int drc_onboard_mic_pr[] = {
	0x8700,
	0x7dc2, 0xa177, 0x5383,
	0x7dc2, 0xa177, 0x5383,
	0x003e, 0x02c1, 0xd37d
};

const unsigned int drc_headset_mic_pr[] = {
	0x8700,
	0x7dc2, 0xa177, 0x5383,
	0x7dc2, 0xa177, 0x5383,
	0x003e, 0x02c1, 0xd37d
...
static const u16 rt5648_reg[RT5648_VENDOR_ID2 + 1] = {
	[RT5648_HP_VOL] = 0xcbcb,
	[RT5648_SPK_VOL] = 0xcbca,
	[RT5648_LOUT1] = 0xc8c8,
...
static int rt5648_agc_drc_mic_get(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_kcontrol_chip(kcontrol);
	struct rt5648_priv *rt5648 = snd_soc_codec_get_drvdata(codec);

	ucontrol->value.integer.value[0] = rt5648_mic_mode;
	pr_debug("%s: value %d\n", __func__, rt5648_mic_mode);

	return 0;
}

static int rt5648_agc_drc_mic_put(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_kcontrol_chip(kcontrol);
	struct rt5648_priv *rt5648 = snd_soc_codec_get_drvdata(codec);
	...
	if (rt5648_mic_mode  == ucontrol->value.integer.value[0])
		return 0;

	rt5648_mic_mode = ucontrol->value.integer.value[0];

	switch (rt5648_mic_mode) {
	case RT5648_DRC_AGC_ONBOARD_MIC:
		pr_info("%s: onboard_mic\n", __func__);
		for (i = 0; i < DRC_PR_REG_SIZE; i++)
			rt5648_index_write(codec, drc_pr_index[i],
				drc_onboard_mic_pr[i]);
		break;
	case RT5648_DRC_AGC_HEADSET_MIC:
		pr_info("%s: headset_mic\n", __func__);
		for (i = 0; i < DRC_PR_REG_SIZE; i++)
			rt5648_index_write(codec, drc_pr_index[i],
				drc_headset_mic_pr[i]);
		break;
	case RT5648_DRC_AGC_MIC_OFF:
		pr_info("%s: off\n", __func__);
		for (i = 0; i < DRC_PR_REG_SIZE; i++)
			rt5648_index_write(codec, drc_pr_index[i],
				drc_off_pr[i]);
...
static int rt5648_eq_get(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_kcontrol_chip(kcontrol);
	struct rt5648_priv *rt5648 = snd_soc_codec_get_drvdata(codec);

	ucontrol->value.integer.value[0] = rt5648_hal_mode;
	pr_debug("%s: value %d\n", __func__, rt5648_hal_mode);

	return 0;
}

static int rt5648_eq_put(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_kcontrol_chip(kcontrol);
	struct rt5648_priv *rt5648 = snd_soc_codec_get_drvdata(codec);
	...
	if (rt5648_hal_mode  == ucontrol->value.integer.value[0])
		return 0;

	rt5648_hal_mode = ucontrol->value.integer.value[0];

	switch (rt5648_hal_mode) {
	case RT5648_EQ_OFF:
		pr_info("%s: off \n", __func__);
...
static const char * const rt5648_eq_mode[] = {
	"MUSIC", "OTHERS", "OFF"
};

static const SOC_ENUM_SINGLE_DECL(rt5648_eq_enum, 0, 0,
		rt5648_eq_mode);

static const char * const rt5648_agc_drc_mic_mode[] = {
	"ONBOARD_MIC", "HEADSET_MIC", "OFF"
};

static const SOC_ENUM_SINGLE_DECL(rt5648_agc_drc_mic_enum, 0, 0,
		rt5648_agc_drc_mic_mode);
...
static const struct snd_kcontrol_new rt5648_snd_controls[] = {
	...
	/* DRC Bass Limit Control */
	SOC_SINGLE("DRC Bass Limit Control", RT5648_DRC2_HL_CTRL2,
		8, 7, 0),
	...
	/* mx f0 */
	SOC_SINGLE("DRC Bass Right Post Boost", RT5648_DRC2_CTRL1, 0, 63, 0),
	SOC_SINGLE("DRC Bass Pre Boost", RT5648_DRC2_CTRL1, 6, 39, 0),
	SOC_SINGLE("DRC Bass Noise Gate Hold Time", RT5648_DRC2_CTRL1, 12, 7, 0),

	/* mx f1 */
	SOC_SINGLE("DRC Bass Recovery Rate Control", RT5648_DRC2_CTRL2, 0, 16, 0),
	SOC_SINGLE("DRC Bass Sample Rate Control", RT5648_DRC2_CTRL2, 5, 7, 0),
	SOC_SINGLE("DRC Bass Attack Rate", RT5648_DRC2_CTRL2, 8, 16, 0),

	/* mx f2 */
	SOC_SINGLE("DRC Bass Noise Gate Ratio", RT5648_DRC2_CTRL3, 0, 3, 0),
	SOC_SINGLE("DRC Bass Drop Enable", RT5648_DRC2_CTRL3, 4, 1, 0),
	SOC_SINGLE("DRC Bass Comp1 Ratio", RT5648_DRC2_CTRL3, 5, 3, 0),
	SOC_SINGLE("DRC Bass Enable", RT5648_DRC2_CTRL3, 7, 1, 0),
	SOC_SINGLE("DRC Bass Left Post Boost", RT5648_DRC2_CTRL3, 8, 63, 0),
	SOC_SINGLE("DRC Bass Comp2 Ratio", RT5648_DRC2_CTRL3, 14, 3, 0),

	/* mx f3 */
	SOC_SINGLE("DRC Bass Noise Gate Boost", RT5648_DRC2_CTRL4, 12, 0x0f, 0),
	SOC_SINGLE("DRC Bass Threshold Min Control", RT5648_DRC2_CTRL4, 5, 0x2e, 0),
	SOC_SINGLE("DRC Bass Noise Gate Threshold", RT5648_DRC2_CTRL4, 0, 0x1f, 0),

	/* mx f4 */
	SOC_SINGLE("DRC Bass Threshold Max", RT5648_DRC2_CTRL5, 0, 0x3f, 0),
	SOC_SINGLE("DRC Bass Threshold2 Max", RT5648_DRC2_CTRL5, 6, 0x3f, 0),
	SOC_SINGLE("DRC Bass Noise Gate Enable", RT5648_DRC2_CTRL5, 13, 1, 0),
	SOC_SINGLE("DRC Bass THMIN Fast Recover Enable", RT5648_DRC2_CTRL5, 14, 1, 0),
	...
	SOC_ENUM_EXT("EQ CONTROL", rt5648_eq_enum,
			rt5648_eq_get, rt5648_eq_put),

	SOC_ENUM_EXT("ACG DRC MIC CONTROL", rt5648_agc_drc_mic_enum,
			rt5648_agc_drc_mic_get, rt5648_agc_drc_mic_put),
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_POST_PMU:
		...
		/* on-board mic and headset mic share the same params */
		if (rt5648_mic_mode == RT5648_DRC_AGC_ONBOARD_MIC) {
			pr_info("%s: mic eq param update\n", __func__);
			rt5648_update_eqmode(codec, EQ_CH_ADC, Z300CL_MIC);
		} else {
			pr_info("%s: mic eq param turned off\n", __func__);
			rt5648_update_eqmode(codec, EQ_CH_ADC, NORMAL);
		}
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_POST_PMU:
		...
		rt5648_update_eqmode(codec, EQ_CH_DACL, Z300CL_SPK);
		rt5648_update_eqmode(codec, EQ_CH_DACR, Z300CL_SPK);
		break;
			...
			enum snd_soc_bias_level level)
{
	...
	case SND_SOC_BIAS_OFF:
		...
		snd_soc_update_bits(codec, RT5648_GEN_CTRL3,
				    0x0600, 0x0000);
		if ((hs_get_status() & SND_JACK_HEADSET) == SND_JACK_HEADSET) {
...
static int rt5648_probe(struct snd_soc_codec *codec)
{
	...
	headset_init();
	rt5648_mic_mode = RT5648_DRC_AGC_MIC_OFF;
	rt5648_hal_mode = RT5648_EQ_OFF;
